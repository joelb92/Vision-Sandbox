...
{
	[self GetMousePositionForEvent:event];
	if(mouseOverController && [mouseOverController.superview isEqual:self]) [mouseOverController.tool StartDragging:[event modifierFlags] & NSShiftKeyMask];
	mouseDragged = false;
}
...
- (void)mouseDragged:(NSEvent*)event
{
	...
	if(mouseOverController && mouseOverController.tool.dragging && [mouseOverController.superview isEqual:self])
	{
		...
		if(!point.isNull())
		{
			[mouseOverController.tool DragTo:point];
		}
	}
